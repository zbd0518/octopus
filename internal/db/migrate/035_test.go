...
// TestMigrateDropLeftoverSiteModelNameUniqueIndex 模拟 34 半成功：
// 新唯一索引已在、旧 CI 唯一索引仍在；35 应删掉旧索引且保留新索引。
...
VALUES (1, 'default', 'GLM-5.2', ?, 'sync')`, key).Error; err != nil {
	...
	if err := migrateDropLeftoverSiteModelNameUniqueIndex(db); err != nil {
		t.Fatalf("migrate 35: %v", err)
	}
	if db.Migrator().HasIndex(&model.SiteModel{}, "idx_site_account_group_model") {
		t.Fatal("leftover old unique index should be dropped by migration 35")
	}
	...
	// 幂等
	if err := migrateDropLeftoverSiteModelNameUniqueIndex(db); err != nil {
		t.Fatalf("second migrate 35: %v", err)
	}
	...
	if err := db.Create(&variant).Error; err != nil {
		t.Fatalf("insert case variant after 35: %v", err)
	}
}
