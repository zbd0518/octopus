...
func init() {
	// 清理 migration 35 半成功残留：新唯一索引已建、旧 CI 唯一索引仍在。
	// 若 35 已标 success 但旧索引未删，同步仍会撞 idx_site_account_group_model。
...
// 036: 删除 site_models 上残留的旧唯一索引 idx_site_account_group_model。
//
...
func migrateDropLeftoverSiteModelNameUniqueIndex(db *gorm.DB) error {
	...
	// 没有 model_name_key 列则说明 35 未生效，交给 35 处理；此处不强行操作。
