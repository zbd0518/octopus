...
以下结论由已完成事项毕业，验证与实现细节见 `../issues/` 中括号内编号对应条目：

- **渠道与分组一致性**：删除渠道（手动 / 过期任务 / 托管通道）统一走同一删除路径，先级联删除受影响分组的分组成员再刷新分组缓存，避免残留已删渠道成员（010）。分组编辑器候选默认隐藏已禁用渠道（可开关显示）、支持按渠道分组筛选、展示静态成员风险徽章；「无可用 Key」只是软提示（仍可选入），「渠道已禁用」才是硬风险（007、012、002、016）。
- **模型治理与测活**：模型/渠道测活提示词可用全局设置 `group_probe_prompt` 配置（默认 `hi`），仅影响测活请求的 chat content 与 embedding input，空白与读取失败回退 `hi`；不影响 Key 巡检（仍调用 `/models`）（008）。测活取 Key 与中继一致遵循「渠道 + Key + 模型」冷却；无可用 Key 的提示区分「无 Key / 全部禁用 / 全部冷却 / 兜底」（006）。
- **中继出站兼容**：OpenAI 兼容的 Chat Completions 出站请求默认不携带 `metadata`（Anthropic 入站 `metadata.user_id` 不泄漏），避免兼容上游返回 `400 Argument not supported: metadata`（003）。
- **观测与日志语义**：客户端主动断开记为中性 `client_closed`，不计入 RequestFailed / 熔断、不写错误日志，有已有进度时仍记成功（006）。
- **站点 / Hub 管理**：站点总览支持按平台类型多选筛选与签到执行状态筛选（全部 / 成功 / 失败 / 未执行 / 禁用），「清空筛选」同时清除搜索、平台、签到三类条件（009）。站点模型以 `model_name_key`（模型名 MD5 指纹）为唯一键持久化，规避 MySQL collation 大小写不敏感下「仅大小写不同模型名」的主键冲突，两种形式可同时保留（001）。
- **号池账号生命周期**：批量导入兼容对象 / 字符串 / 其他合法 JSON 的 `extra`，导出与导入格式互兼容；Sub2API OpenAI OAuth 导入与账号编辑支持凭据展示与代理选择修改；支持账号批量删除与一键清空（015、017、018）。
- **内存与可靠性**：流式中继入站按帧在线增量聚合，不在请求内缓存全部 SSE chunk 对象，避免长流 / 大图并发时内存线性放大；聚合结果在请求结束时产出（005）。
...
- 历史 issue 与功能记录 → `../issues/`（已对齐编号 `001-x-`…`018-x-`）